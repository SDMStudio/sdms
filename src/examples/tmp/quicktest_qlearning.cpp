...
int learn(int argv, char **args)
{
    try
    {
        ...
        if (formalism == "MDP")
            ...
            gym = std::make_shared<PrivateHierarchicalOccupancyMDP>(dpomdp, memory, batch_size, true, false);
        else if ((formalism == "PrivateHierarchicalOccupancyMDP") && (qvalue == "hierarchical"))
            gym = std::make_shared<PrivateHierarchicalOccupancyMDP>(dpomdp, memory, batch_size, true, true);
