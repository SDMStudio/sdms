...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		auto serialized_oMDP = std::make_shared<SerializedOccupancyMDP<TState, TAction>>(filename, 2);

		auto p_algo = sdm::algo::makeHSVI<TState, TAction>(serialized_oMDP, "", "", "MaxInitializer", "MinInitializer", 1, 0, 2, 100,"	");
        
		p_algo->do_initialize();

		std::cout<<"\n getLowerBOund "<<p_algo->getLowerBound()->str();
		std::cout<<"\n getUpperBOund "<<p_algo->getUpperBound()->str();
