...
namespace sdm
{
    ...
    std::shared_ptr<State> MaxPlanBackup::setHyperplanOccupancy(const std::shared_ptr<ValueFunction>& vf, const std::shared_ptr<State> &belief_state, const std::shared_ptr<Action> &decision_rule, number t)
    {
        ...
        auto next_occupancy = std::make_shared<OccupancyState>(*std::dynamic_pointer_cast<OccupancyState>(std::dynamic_pointer_cast<OccupancyStateGraph>(occupancy)->getData()->toOccupancyState()));
        ...
        for(const auto &jhistory : occupancy->getFullyUncompressedOccupancy()->getJointHistories())
        {
            ...
            auto next_occupancy_state = occupancy_mdp->nextOccupancy(occupancy, decision_rule , nullptr,t);
