...
namespace sdm
{
    ...
    TabularQValueBackup::TabularQValueBackup(
        ...
        double discount,
        number horizon
    ) : experience_memory_(std::static_pointer_cast<ExperienceMemory>(experience_memory)), q_value_table_(q_value_table), target_q_value_table_(target_q_value_table), discount_(discount), horizon_(horizon)
    {
    ...
    double TabularQValueBackup::update()
    {   
        for (number t = 0; t < this->horizon_; t++)
        {
            auto [_, observation, action, reward, next_observation, __] = this->experience_memory_->sample(t)[0];
