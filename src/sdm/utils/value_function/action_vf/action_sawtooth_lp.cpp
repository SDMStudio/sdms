...
namespace sdm
{
    ...
    ActionVFSawtoothLP::ActionVFSawtoothLP(const std::shared_ptr<SolvableByHSVI>& world,TypeOfResolution current_type_of_resolution, number bigM_value, TypeSawtoothLinearProgram type_sawtooth_resolution) : 
        ActionVFBase<double>(world),DecentralizedLPOccupancy(world) , csawtooth_lp_(type_sawtooth_resolution), current_type_of_resolution_(current_type_of_resolution)
    {
    ...
    Pair<std::shared_ptr<Action>,double> ActionVFSawtoothLP::selectBestAction(const std::shared_ptr<ValueFunction>& vf, const std::shared_ptr<State>& state, number t)
    {
        ...
        case TypeSawtoothLinearProgram::PLAIN_SAWTOOTH_LINER_PROGRAMMING:
            return this->greedyFullSawtooth(vf,state, t);
        case TypeSawtoothLinearProgram::RELAXED_SAWTOOTH_LINER_PROGRAMMING:
            return this->greedyRelaxedSawtooth(vf,state, t);
        default:
            return this->greedyFullSawtooth(vf,state, t);
        }
    }
    ...
    Pair<std::shared_ptr<Action>,double> ActionVFSawtoothLP::createFullSawtooth(const std::shared_ptr<ValueFunction>&,const std::shared_ptr<State> &state, number t)
    {
        this->tmp_representation = this->representation[t + 1];
        return this->createLP(state, t);
    }

    void ActionVFSawtoothLP::createVariables(const std::shared_ptr<State> &state, IloEnv &env, IloNumVarArray &var, number t)
    {
        try
        {
            ...
            this->createDecentralizedVariables(state, env, var, index, t);
        }
    ...
    double ActionVFSawtoothLP::getQValueRelaxation(const  std::shared_ptr<OccupancyStateInterface> &compressed_occupancy_state, const std::shared_ptr<JointHistoryInterface> joint_history, std::shared_ptr<Action> action, number t)
    {
    ...
    void ActionVFSawtoothLP::createConstraints(const std::shared_ptr<State>&state, IloEnv &env, IloRangeArray &con, IloNumVarArray &var, number &index, number t)
    {
       ...
       try{
            this->setInitialConstrainte(state, env, con, var, index, t);
    ...
    //             if (state->getTypeState() == TypeState::SERIAL_OCCUPANCY_STATE)
    //             {
    //                 under_pb = std::dynamic_pointer_cast<SerialMMDP>(this->world_->getUnderlyingProblem());
    ...
    //         // <! \omega_k(x',o') * BigM
    //         auto VarName = this->getVarNameWeightedStateJointHistory(next_state, next_hidden_state, next_joint_history);
    ...
    double ActionVFSawtoothLP::getQValueRealistic(const std::shared_ptr<State> &state, const std::shared_ptr<JointHistoryInterface>& joint_history, const std::shared_ptr<Action>& action, std::shared_ptr<State> next_hidden_state, const std::shared_ptr<Observation> next_observation, double denominator, double difference)
    {
        return difference * this->getSawtoothMinimumRatio(state, joint_history, action, next_hidden_state, next_observation, denominator);
    }


    // void ActionVFSawtoothLP::setGreedySawtoothIloIfThen(const std::shared_ptr<State> &state, const std::shared_ptr<JointHistoryInterface>& joint_history, const std::shared_ptr<State> &next_hidden_state, const std::shared_ptr<Observation> &next_observation, const std::shared_ptr<JointHistoryInterface> &next_joint_history, const std::shared_ptr<State> &next_state, double probability, double difference, IloEnv &env, IloModel &model, IloNumVarArray &var,number t)
    // {
    //     number recover = 0;
    ...
    double ActionVFSawtoothLP::getSawtoothMinimumRatio(const std::shared_ptr<State> &state, const std::shared_ptr<JointHistoryInterface>& joint_history, const std::shared_ptr<Action>& action, const std::shared_ptr<State>& next_hidden_state, const std::shared_ptr<Observation>& next_observation, double denominator)
    {
        try
        {
            auto factor = 0.0;
            ...
            for (const auto &hidden_state : compressed_occupancy_state->getStatesAt(joint_history))
            {
                ...
                factor += compressed_occupancy_state->getProbability(compressed_occupancy_state->HiddenStateAndJointHistoryToState(hidden_state, joint_history) * under_pb->getDynamics(hidden_state, action, next_observation, next_hidden_state);
            }
        }
        catch (const std::exception &exc)
        {
