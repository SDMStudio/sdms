...
namespace sdm
{
    ...
    ActionVFMaxplanSerial::ActionVFMaxplanSerial(const std::shared_ptr<SolvableByHSVI>& world): ActionVFBase<std::shared_ptr<State>>(world) {}

    Pair<std::shared_ptr<Action>,std::shared_ptr<State>> ActionVFMaxplanSerial::selectBestAction(const std::shared_ptr<ValueFunction>& vf, const std::shared_ptr<State>& state, number t)
    {
