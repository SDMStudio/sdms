...
namespace sdm
{
    ...
    Pair<std::shared_ptr<Action>,std::shared_ptr<State>> ActionVFMaxplanLP::selectBestAction(const std::shared_ptr<ValueFunction>& vf, const std::shared_ptr<State>& state, number t)
    {
        ...
        for (const auto &hyperplan : vf->getSupport(t + 1))
        {
            ...
            auto pair_action_value = this->createLP(state->toOccupancyState(), t);
    ...
    void ActionVFMaxplanLP::createObjectiveFunction(const std::shared_ptr<State> &occupancy_state, IloNumVarArray &var, IloObjective &obj, number t)
    {
