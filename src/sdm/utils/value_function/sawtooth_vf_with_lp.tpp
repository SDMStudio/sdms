...
namespace sdm
{
    ...
    template <typename TState, typename TAction, typename TValue>
    TAction SawtoothValueFunctionLP<TState, TAction, TValue>::getBestAction(const TState &occupancy_state, number t)
    {
        double clb =0, cub = 0;
        return this->greedySawtooth(occupancy_state, clb, cub, t);
    }
    ...
    void SawtoothValueFunctionLP<TState, TAction, TValue>::setGreedyVariables(const TState& occupancy_state, std::unordered_map<agent, std::unordered_set<typename TState::jhistory_type::element_type::ihistory_type>>& ihs, IloEnv& env, IloNumVarArray& var, double /*clb*/, double /*cub*/, number t)
    {
        ...
        var.add(IloNumVar(env, -IloInfinity, 0, VarName.c_str())); 
    ...
    template <typename TState, typename TAction, typename TValue>
    double SawtoothValueFunctionLP<TState, TAction, TValue>::getQValueRealistic(const TState& compressed_occupancy_state, typename TState::jhistory_type joint_history, typename TAction::output_type action, typename TState::state_type next_hidden_state, typename TState::observation_type next_observation, const TState& next_one_step_uncompressed_occupancy_state, double difference, number )
    {
        // auto upper_bound = this->getQValueRelaxation(compressed_occupancy_state, joint_history, action, t);
        
        return difference * this->template getSawtoothMinimumRatio<TState>(*compressed_occupancy_state.getOneStepUncompressedOccupancy(), joint_history, action, next_hidden_state, next_observation, next_one_step_uncompressed_occupancy_state);
    }
            ...
            for(const auto &pair_hidden_state_AND_joint_history_AND_probability : *next_one_step_uncompressed_occupancy_state)
            {
                ...
                if(occupancy_state.getJointHistories().find(joint_history) != occupancy_state.getJointHistories().end())
                {
                    ...
                    for(const auto & action : this->getWorld()->getUnderlyingProblem()->getActionSpace()->getAll())
                    {
                        ...
                        expr -= this->getQValueRealistic(occupancy_state, joint_history, action, next_hidden_state, next_observation, *next_one_step_uncompressed_occupancy_state, difference, t) * var[this->getNumber(this->getVarNameJointHistoryDecisionRule(action, joint_history))];
                    } 
    ...
    template <typename TState, typename TAction, typename TValue>
    template <typename T, std::enable_if_t<std::is_same_v<SerializedOccupancyState<>, T>, int>>
    void SawtoothValueFunctionLP<TState, TAction, TValue>::setGreedyUpperbound(const TState&, IloEnv& env, IloRangeArray& con, IloNumVarArray& var, number& c, number ) 
    {
        //<! tracking variable ids
        number recover;
    ...
    double SawtoothValueFunctionLP<TState, TAction, TValue>::getSawtoothMinimumRatio(const TState& one_step_uncompressed_occupancy_state, typename TState::jhistory_type joint_history, typename TAction::output_type action, typename TState::state_type next_hidden_state, typename TState::observation_type next_observation, const TState& next_one_step_uncompressed_occupancy_state)
    {
        ...
        for(const auto& hidden_state : one_step_uncompressed_occupancy_state.getStatesAt(joint_history))
        {
            factor += one_step_uncompressed_occupancy_state.at(std::make_pair(hidden_state, joint_history)) * this->getWorld()->getUnderlyingProblem()->getDynamics(hidden_state, action, next_observation, next_hidden_state);
        }
        return factor / next_one_step_uncompressed_occupancy_state.at(std::make_pair(next_hidden_state, joint_history->expand(next_observation)));
    }
}
