...
namespace sdm
{
    ...
    void SawtoothValueFunctionLP<TState, TAction, TValue>::updateValueAt(const TState &occupancy_state, number t)
    {
        ...
        {
            // catch anything thrown within try block that derives from std::exception
            std::cerr << "SawtoothValueFunctionLP<TState, TAction, TValue>::updateValueAt(..) exception caught: " << exc.what() << std::endl;
            exit(-1);
        }
    }
    ...
    TValue SawtoothValueFunctionLP<TState, TAction, TValue>::getValueAt(const TState &occupancy_state, number t)
    {
        ...
        {
            // catch anything thrown within try block that derives from std::exception
            std::cerr << "SawtoothValueFunctionLP<TState, TAction, TValue>::getValueAt(..) exception caught: " << exc.what() << std::endl;
            exit(-1);
        }
    }
