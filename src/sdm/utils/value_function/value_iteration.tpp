...
namespace sdm
{
    ...
    template <typename TState, typename TAction>
    std::shared_ptr<typename sdm::MappedValueFunction<TState, TAction>> ValueIteration<TState,TAction>::policy_iteration_non_inf()
    {
        auto under_pb = this->problem_->getUnderlyingProblem();

        auto policy_evaluation = std::make_shared<sdm::MappedValueFunction<TState, TAction>>(this->problem_, under_pb->getPlanningHorizon(),under_pb->getReward()->getMaxReward());
        auto policy_evaluation_tau1 = std::make_shared<sdm::MappedValueFunction<TState, TAction>>(this->problem_, under_pb->getPlanningHorizon(),under_pb->getReward()->getMaxReward());
    ...
    template <typename TState, typename TAction>
    bool ValueIteration<TState,TAction>::borne(std::shared_ptr<sdm::MappedValueFunction<TState, TAction>> policy_evaluation_tempo,std::shared_ptr<sdm::MappedValueFunction<TState, TAction>> policy_evaluation,double epsilon)
    {
