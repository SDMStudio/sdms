...
namespace sdm
{
    ...
    ValueIteration<TState,TAction>::ValueIteration(std::shared_ptr<SolvableByHSVI<TState, TAction>> problem,double discount, double error, int horizon) : problem_(problem), error_(error)    
    {
    ...
    void ValueIteration<TState,TAction>::do_initialize()
    {
        ...
        policy_evaluation_1_ = std::make_shared<sdm::MappedValueFunction<TState, TAction>>(this->problem_, under_pb->getPlanningHorizon(),under_pb->getReward()->getMaxReward());
        policy_evaluation_2_ = std::make_shared<sdm::MappedValueFunction<TState, TAction>>(this->problem_, under_pb->getPlanningHorizon(),under_pb->getReward()->getMaxReward());
