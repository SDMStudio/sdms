namespace sdm
{
  ...
  TAction MaxPlanValueFunctionLP<TVector, TAction, TValue>::greedyMaxPlane(const TVector &occupancy_state, const TVector &hyperplan, double &value, double, number t)
  {
    ...
    try
    {
      ...
      this->template setDecentralizedConstraints<TVector>(occupancy_state, env, con, var, c, t);
  ...
  template <typename TVector, typename TAction, typename TValue>
  void MaxPlanValueFunctionLP<TVector, TAction, TValue>::setGreedyVariables(const TVector &occupancy_state, IloEnv &env, IloNumVarArray &var, number t)
  {
    ...
    this->template setDecentralizedVariables<TVector>(occupancy_state, env, var, index, t);
  }
  ...
  void MaxPlanValueFunctionLP<TVector, TAction, TValue>::setGreedyObjective(const TVector &occupancy_state, IloNumVarArray &var, IloObjective &obj, const TVector &hyperplan, number t)
  {
    ...
    for (const auto &joint_history : occupancy_state.getJointHistories())
    {
      ...
      for (const auto &joint_action : underlying_problem->getActionSpace()->getAll())
      {
        weight = 0.0;

        for (const auto &belief : occupancy_state.getStatesAt(joint_history))
        {
          //<! 1.a compute factor
          factor = (belief->getData() ^ underlying_problem->getReward()->getReward(underlying_problem->getActionSpace()->joint2single(joint_action)));

          if (t < underlying_problem->getPlanningHorizon() - 1)
          {
            for (const auto &next_observation : underlying_problem->getObsSpace()->getAll())
            {
              auto next_joint_history = joint_history->expand(next_observation);
              auto next_belief = belief->expand(underlying_problem->getActionSpace()->joint2single(joint_action), underlying_problem->getObsSpace()->joint2single(next_observation));

              factor += belief->getProbability(underlying_problem->getActionSpace()->joint2single(joint_action), underlying_problem->getObsSpace()->joint2single(next_observation)) * hyperplan.at({next_belief, next_joint_history});
            }
          }

          weight += occupancy_state.at(std::make_pair(belief, joint_history)) * factor;
        }

        //<! 1.b get variable a(u|o)
        recover = this->getNumber(this->getVarNameJointHistoryDecisionRule(joint_action, joint_history));
  ...
  void MaxPlanValueFunctionLP<TVector, TAction, TValue>::setGreedyObjective(const TVector &occupancy_state, IloNumVarArray &var, IloObjective &obj, const TVector &hyperplan, number t)
  {
    ...
    for (const auto &joint_history : occupancy_state.getJointHistories())
    {
      ...
      for (const auto &action : this->getWorld()->getUnderlyingProblem()->getActionSpace()->getAll())
      {
        ...
        for (const auto &hidden_state : this->getWorld()->getUnderlyingProblem()->getStateSpace()->getAll())
        {
          //<! 1.a compute factor
          ...
          {
            for (const auto &next_hidden_state : this->getWorld()->getUnderlyingProblem()->getReachableStates(hidden_state, action))
            {
              for (const auto &next_observation : this->getWorld()->getUnderlyingProblem()->getReachableObservations(hidden_state, action, next_hidden_state))
              {
          ...
          weight += occupancy_state.at(std::make_pair(hidden_state, joint_history)) * factor;
        }
