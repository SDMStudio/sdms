...
namespace sdm
{
    ...
    void POMDPInitializer<TState, TAction>::init(std::shared_ptr<ValueFunction<TState, TAction>> vf)
    {
        ...
        auto algorithm = algo::makeHSVI<decltype(pomdp->getInitialState()), number>(pomdp, "tabular", "tabular", "MdpHsviInitializer", "MinInitializer", underlying_pb->getDiscount(), this->error_, underlying_pb->getPlanningHorizon(), this->trials_, "pomdp_init", "BigM", 100, "Full", 1, 1);
