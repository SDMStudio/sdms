...
namespace sdm
{
    ...
    {
        auto pomdp = std::static_pointer_cast<typename WorldType<TState, TAction>::type>(vf->getWorld())->toPOMDP();
        auto underlying_pb = pomdp->getUnderlyingProblem();
        auto algorithm = algo::makeMappedHSVI<decltype(pomdp->getInitialState()), number>(pomdp, "MDPInitializer", "MinInitializer", underlying_pb->getDiscount(), this->error_, underlying_pb->getPlanningHorizon(), this->trials_, "pomdp_init");
        algorithm->do_solve();
