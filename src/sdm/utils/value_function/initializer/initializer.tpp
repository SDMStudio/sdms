...
namespace sdm
{
    ...
    ValueInitializer<TInput>::ValueInitializer(double v) : value(v){}

    template <class TInput>
    void ValueInitializer<TInput>::initBase(std::shared_ptr<BaseValueFunction> vf)
    {
        if (vf->getHorizon() < 1)
        {
    ...
    template <class TInput>
    void ValueInitializer<TInput>::init(std::shared_ptr<ValueFunction> vf)
    {
        this->initBase(vf);
    }

    template <class TInput>
    void ValueInitializer<TInput>::init(std::shared_ptr<QValueFunction<TInput>> vf)
    {
        this->initBase(vf);
    }
    ...
    // ****************** 
    template <class TInput>
    ZeroInitializer<TInput>::ZeroInitializer(std::shared_ptr<SolvableByHSVI>) : ValueInitializer<TInput>(0){}
