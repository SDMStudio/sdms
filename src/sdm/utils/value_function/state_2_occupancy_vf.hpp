...
namespace sdm
{
    ...
    public:
        ...
        template <bool is_mdp = std::is_any<typename WorldType<TOccupancyState>::type, DiscreteMDP,SerializedMMDP<SerializedState, number>>::value>
        std::enable_if_t<is_mdp, double>
        operator()(const TOccupancyState &ostate, const number &tau);

        // template <bool is_serial_mdp = std::is_any<typename WorldType<TOccupancyState>::type, SerializedMMDP<SerializedState, number>>::value>
