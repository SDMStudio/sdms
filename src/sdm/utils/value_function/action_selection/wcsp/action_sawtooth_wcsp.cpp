...
// namespace sdm
// {
//     ActionSelectionSawtoothWCSP::ActionSelectionSawtoothWCSP(){}

//     ActionSelectionSawtoothWCSP::ActionSelectionSawtoothWCSP(const std::shared_ptr<SolvableByHSVI>& world): ActionSelectionBase(world) {}

//     Pair<std::shared_ptr<Action>, double> ActionSelectionSawtoothWCSP::getGreedyActionAndValue(const std::shared_ptr<ValueFunctionInterface>& vf, const std::shared_ptr<State>& state, number t)
//     {
...
//             for (const auto &point_AND_value : value_function->getRepresentation(t + 1))
//             {
...
//                         auto [action,value] = this->createWCSPProblem(value_function,state, t);
...
//     Pair<std::shared_ptr<Action>,double>  ActionSelectionSawtoothWCSP::createWCSPProblem(const std::shared_ptr<ValueFunctionInterface>& vf , const std::shared_ptr<State>& state, number t)
//     {
...
//     double ActionSelectionSawtoothWCSP::getValueAt(const std::shared_ptr<ValueFunction>& vf,const std::shared_ptr<OccupancyStateInterface>& occupancy_state, const std::shared_ptr<JointHistoryInterface>& joint_history, const std::shared_ptr<Action>& action,const std::shared_ptr<State>& , number t)
//     {
//         double sawtooth_ratio = 0.0;

//         auto underlying_problem = std::dynamic_pointer_cast<MPOMDPInterface>(this->world_->getUnderlyingProblem());
...
//             {
//                 numerator +=  occupancy_state->getProbability(joint_history,hidden_state) * underlying_problem->getDynamics(hidden_state,action,next_hidden_state,next_joint_observation,t);
//             }
...
//     Cost ActionSelectionSawtoothWCSP::getCost(double value)
//     {
...
//     std::set<std::shared_ptr<JointHistoryInterface>> ActionSelectionSawtoothWCSP::determineJointHistory(const std::shared_ptr<State> &state)
//     {
