...
//     {
//         auto underlying_problem = std::dynamic_pointer_cast<SerialMPOMDPInterface>(ActionSelectionBase::world_->getUnderlyingProblem());

//         // Check if last agent
//         bool last_agent = std::dynamic_pointer_cast<SerialProblemInterface>(ActionSelectionBase::world_)->isLastAgent(t);

//         // Get next observation
//         auto next_joint_observation = (last_agent) ? std::static_pointer_cast<Joint<std::shared_ptr<Observation>>>(next_joint_history->getLastObservation()) : underlying_problem->getDefaultObservation();
