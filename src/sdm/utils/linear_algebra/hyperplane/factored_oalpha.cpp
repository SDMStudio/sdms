...
namespace sdm
{
    Factored_oAlpha::Factored_oAlpha(double default_value) : AlphaVector(default_value)
    {
    }

    bool Factored_oAlpha::isDominated(const Hyperplane &other) const
    {
    ...
    double Factored_oAlpha::getValueAt(const std::shared_ptr<State> &x, const std::shared_ptr<HistoryInterface> &o) const
    {
    ...
    void Factored_oAlpha::setValueAt(const std::shared_ptr<State> &x, const std::shared_ptr<HistoryInterface> &o, double value)
    {
        auto iter = this->repr.find(o);
        if (iter == this->repr.end())
        {
    ...
    double Factored_oAlpha::getBetaValueAt(const std::shared_ptr<State> &x, const std::shared_ptr<HistoryInterface> &o, const std::shared_ptr<Action> &u, const std::shared_ptr<POMDPInterface> &pomdp, number t)
    {
    ...
    size_t Factored_oAlpha::hash(double precision) const
    {
        if (precision < 0)
            precision = Factored_oAlpha::PRECISION;
    ...
    bool Factored_oAlpha::isEqual(const Factored_oAlpha &other, double precision) const
    {
    ...
    bool Factored_oAlpha::isEqual(const std::shared_ptr<Hyperplane> &other, double precision) const
    {
        auto other_oalpha = std::static_pointer_cast<Factored_oAlpha>(other);
    ...
    size_t Factored_oAlpha::size() const
    {
        return this->repr.size();
    }

    std::string Factored_oAlpha::str() const
    {
    ...
    AlphaGroup &Factored_oAlpha::getAlphaGroup(int group_id)
    {
