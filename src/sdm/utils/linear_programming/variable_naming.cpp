...
namespace sdm
{
  ...
  {
    std::ostringstream oss;
    oss << "idr." << action << "." << individual_history << "." << agent_id;
    return oss.str();
  }
  ...
  std::string VarNaming::getVarNameWeightedStateJointHistory(const std::shared_ptr<State> &s, const std::shared_ptr<State> &x, const std::shared_ptr<JointHistoryInterface> &o)
  {
    std::ostringstream oss;
    oss << "wsh." << s << "." << x << "." << o;
    return oss.str();
