...
namespace sdm
{
  ...
  class QLearningBaris : public Algorithm
  {
  private:
  ...
  protected:
     ...
     * 
     */
    std::shared_ptr<JointHistoryPrivateOccupancyMDP<TObservation, TAction>> env_;

    /**
     * @brief Q-value function. 
     */
    std::shared_ptr<QValueFunction<TObservation, Joint<number>>> q_value_;

    /**
     * @brief Q-value target function. 
     */
    std::shared_ptr<QValueFunction<TObservation, Joint<number>>> q_target_;
  ...
  public:
    QLearningBaris(std::shared_ptr<JointHistoryPrivateOccupancyMDP<TObservation, TAction>> &env,
    ...
    TAction select_greedy_action(const TObservation &obs);

    void initLogger();
  };
} // namespace sdm
#include <sdm/algorithms/q_learning_baris.tpp>
