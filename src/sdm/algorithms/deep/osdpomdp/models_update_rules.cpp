...
namespace sdm{
	...
	double ModelsUpdateRules::update(
		...
		torch::Tensor o2_batch, o1_batch, all_o1s_batch, u_batch, r_batch, next_o2_batch, next_o1_batch, next_all_o1s_batch;
		std::vector<torch::Tensor> next_all_o1_batches;
		// Std::tie the Tensors appropriately. 
		std::tie(o2_batch, o1_batch, all_o1s_batch, u_batch, r_batch, next_o2_batch, next_o1_batch, next_all_o1s_batch, next_all_o1_batches) = b; 
		//
		torch::Tensor q_values = get_q_values(o2_batch, o1_batch, all_o1s_batch, u_batch, agents->policy_net);
		//
		torch::Tensor target_q_values = get_target_q_values(next_o2_batch, next_o1_batch, next_all_o1s_batch, r_batch, agents, alpha);
		//
	...
	torch::Tensor ModelsUpdateRules::get_target_q_values(
		...
		torch::Tensor next_state_values;
		{
			...
			if (!induced_bias){
				...
				torch::Tensor no2_no1_nao1s_batch = torch::cat({next_o2_batch, next_o1_batch, next_all_o1s_batch}, 1);
				// {V}_{t+1}^{1} = \max_{a'}{PolicyNet}^{Agent1}(o_{t+1}^{0}, \{o_{t+1}^{1, (m)}\}, a_{t+1}^{0}, a', \{o_{t+1}^{1}\})
				next_state_values = std::get<0>(agents->target_net(no2_no1_nao1s_batch).max(1));
	...
	batch ModelsUpdateRules::construct_batch(std::vector<transition> transitions){
		...
		std::vector<std::vector<torch::Tensor>> next_all_o1_batches_vector(K); // sampling_memory x batch_size x dim_o1
		...
		for (transition t: transitions){
			...
			for (int k = 0; k < K; k++){
				...
				next_all_o1_batches_vector[k].push_back(next_all_o1);
			}
		}
		...
		std::vector<torch::Tensor> next_all_o1_batches;
		for (int k = 0; k < K; k++){
			std::vector<torch::Tensor> next_one_o1_batch_vector = next_all_o1_batches_vector[k];
		...
		return std::make_tuple(o2_batch, o1_batch, all_o1s_batch, u_batch, r_batch, next_o2_batch, next_o1_batch, next_all_o1s_batch, next_all_o1_batches);
	}
}