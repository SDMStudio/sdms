...
namespace sdm
{
    ...
    void AlphaStar::do_solve()
    {
        ...
        while (this->openSet.size() != 0 && !this->do_stop(*this->openSet.begin(),0, this->openSet.begin()->get()->horizon_))
        {
            ...
            this->do_explore(*this->openSet.begin(), 0, this->openSet.begin()->get()->horizon_);
    ...
    bool AlphaStar::do_stop(const std::shared_ptr<State> &, double , number h)
    {
        return this->world_->getUnderlyingProblem()->getHorizon() <= h || this->openSet.size() == 0;
    }

    void AlphaStar::do_explore(const std::shared_ptr<State> &state_tmp, double cost_so_far, number h)
    {
        try
        {
            if (!this->do_stop(state_tmp, cost_so_far, h))
            {
        ...
        catch (const std::exception &exc)
        {
            ...
            std::cerr << "AlphaStar::do_explore(..) exception caught: " << exc.what() << std::endl;
            exit(-1);
        }
    }

    void AlphaStar::do_test()
    {
    }

    void AlphaStar::do_save()
    {
    }
    ...
    void AlphaStar::saveResults(std::string , double )
    {
    }

    double AlphaStar::getResult()
    {
        return this->bound_->getValueAt(this->start_state);
    }
