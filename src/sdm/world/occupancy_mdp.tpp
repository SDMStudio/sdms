...
namespace sdm
{
    ...
    template <class TOccupancyState>
    std::shared_ptr<ObservationSpace> BaseOccupancyMDP<TOccupancyState>::getObservationSpaceAt(const std::shared_ptr<State> &, const std::shared_ptr<Action> &, number)
    {
        return std::make_shared<DiscreteObservationSpace>(std::vector({sdm::NO_OBSERVATION}));
    }
    ...
    template <class TOccupancyState>
    std::shared_ptr<ActionSpace> BaseOccupancyMDP<TOccupancyState>::getActionSpaceAt(const std::shared_ptr<State> &ostate, number t)
    {
        ...
        if (occupancy_state->getActionSpaceAt(t) == nullptr)
        {
            ...
            std::shared_ptr<ActionSpace> joint_ddr_space = this->computeActionSpaceAt(ostate, t);
    ...
    {
        if (this->store_actions_)
            return this->getActionSpaceAt(ostate, t)->sample();
        else
    ...
    template <class TOccupancyState>
    std::shared_ptr<ActionSpace> BaseOccupancyMDP<TOccupancyState>::computeActionSpaceAt(const std::shared_ptr<State> &ostate, number t)
    {
        // return ostate->getActionSpace(ostate);

        // Vector of individual deterministic decision rules of each agent.
        std::vector<std::shared_ptr<DiscreteActionSpace>> individual_ddr_spaces;
        ...
        for (int agent = 0; agent < this->mdp->getNumAgents(); agent++)
        {
            ...
            std::shared_ptr<DiscreteSpace<std::shared_ptr<HistoryInterface>>> individual_history_space = std::make_shared<DiscreteSpace<std::shared_ptr<HistoryInterface>>>(sdm::tools::set2vector(individual_descriptive_statistics));
            // Get individual ddr of agent i.
            std::shared_ptr<DiscreteActionSpace> individual_ddr_space = std::make_shared<FunctionSpace<DeterministicDecisionRule, Action>>(individual_history_space, this->decpomdp->getActionSpace(agent, t), this->store_actions_);
        ...
        std::shared_ptr<ActionSpace> joint_ddr_space = std::make_shared<FunctionSpace<JointDeterministicDecisionRule, Action>>(
            std::make_shared<DiscretePtrSpace<HistoryInterface>>(std::vector<std::shared_ptr<HistoryInterface>>{nullptr}),
            std::make_shared<MultiDiscreteActionSpace>(individual_ddr_spaces, this->store_actions_),
            this->store_actions_,
    ...
    //         auto next_compressed_joint_history = compressed_joint_history->expand(observation);
