...
namespace sdm
{
    ...
    {
        // Initialize history
        this->initial_history_ = std::make_shared<JointHistoryTree>(this->getUnderlyingMPOMDP()->getNumAgents(), (history_length > 0) ? history_length : -1);

        // Initialize empty state
        auto initial_state = std::make_shared<OccupancyState>(this->getUnderlyingMPOMDP()->getNumAgents());

        initial_state->setProbabilityAt(this->initial_history_, 1);
        initial_state->setBeliefAt(this->initial_history_, this->initial_state_->toBelief());
        ...
        try
        {
            ...
            for (const auto &joint_history : ostate->getFullyUncompressedOccupancy()->getJointHistories())
            {
                ...
                for (const auto &belief : ostate->getBeliefsAt(joint_history))
                {
                    ...
                    {
                        // p(o') = p(o) * p(z | b, a)
                        double proba_next_history = ostate->getProbability(joint_history) * BeliefMDP::getObservationProbability(belief, joint_action, joint_observation, nullptr, t);
                        ...
                        if (proba_next_history > 0)
                        {
                            ...
                            new_fully_uncompressed_occupancy_state->setProbabilityAt(next_joint_history, proba_next_history);
                            new_fully_uncompressed_occupancy_state->setBeliefAt(next_joint_history, next_belief);
                            ...
                            new_one_step_left_compressed_occupancy_state->addProbabilityAt(next_compressed_joint_history, proba_next_history);
                            new_fully_uncompressed_occupancy_state->setBeliefAt(next_compressed_joint_history, next_belief);
    ...
    double OccupancyMDP::getReward(const std::shared_ptr<State> &ostate, const std::shared_ptr<Action> &joint_idr) const
    {
        ...
        for (const auto &joint_history : ostate->toOccupancyState()->getJointHistories())
        {
            ...
            reward += ostate->toOccupancyState()->getProbability(joint_history) * BeliefMDP::getReward(ostate->toOccupancyState()->getBeliefAt(joint_history), jaction);
