...
namespace sdm{
	struct ModelsUpdateRules {
		...
		torch::Tensor get_next_u2_batch(torch::Tensor, torch::Tensor, std::vector<torch::Tensor>, torch::Tensor, DQN&);
		// {Q}_{t}^{1} = {Q}^{1}(o_{t}^{0}, o_{t}^{1, (n)}, a_{t}^{0}, a_{t}^{1, (n)}, \{o_{t}^{1, (m))}\}_{m=0}^{M-1})
		torch::Tensor get_q_values(torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, DQN&);
		...
		torch::Tensor get_target_q_values(torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, std::shared_ptr<Agents>&, float);
		// Update the parameters of agent 1's policy net.
		void update_agent_1_policy_net(std::shared_ptr<Agents>&, torch::Tensor);
	};
}