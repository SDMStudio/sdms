...
namespace sdm
{
    ...
    number DiscreteMDP::nextState(number state, number action, int t, HSVI<number, number> *hsvi) const
    {
    ...
    std::shared_ptr<DiscreteSpace<number>> DiscreteMDP::getActionSpaceAt(number state)
    {
        return this->getActionSpace();
    }

    double DiscreteMDP::getReward(number state, number action) const
    {
        return this->getReward()->getReward(state, action);
    }

    double DiscreteMDP::getExpectedNextValue(ValueFunction<number, number> *value_function, number state, number action, int t) const
    {
        double tmp = 0;
        for (number state_ = 0; state_ < this->getStateSpace()->getNumItems(); state_++)
        {
