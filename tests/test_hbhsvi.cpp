#define BOOST_TEST_MODULE hbHsviTest
...
BOOST_AUTO_TEST_CASE(Solver_hier_bMDP)
{
    auto algorithm = make("mabc.dpomdp", "hbMDP", 10);
    ...
    algorithm = make("recycling.dpomdp", "hbMDP", 5);
    ...
    algorithm = make("tiger.dpomdp", "hbMDP", 3);
    algorithm->getUpperBound()->setInitializer(sdm::initializer::registry::make("Mdp", algorithm->getWorld()));
    algorithm->initialize();
    algorithm->solve();
    resultat = algorithm->getLowerBound()->getValueAt(algorithm->getWorld()->getInitialState());
