...
int main(int argc, char **argv)
{
    ...
    options.add_options()("num_agents,n", po::value<number>(&num_agents)->default_value(2), "the number of agents")("action,a", po::value<std::vector<number>>(&list_num_action)->multitoken(), "list of number of observation")("list_num_histories,h", po::value<std::vector<number>>(&list_num_histories)->multitoken(), "list of number of individual histories")("truncation,t", po::value<number>(&truncation)->default_value(-1), "the truncation parameter (for history)");
    ...
    auto history_space = std::make_shared<MultiDiscreteSpace>(history_spaces);
    auto action_space = std::make_shared<MultiDiscreteSpace>(action_spaces);
    ...
    for (int agent_id = 0; agent_id < num_agents; agent_id++)
    {
        ...
        vector_indiv_space.push_back(std::make_shared<FunctionSpace<DeterministicDecisionRule>>(history_space_i, action_space_i));
    }
    ...
    {
        std::cout << "\033[1m\033[36m  <decision id=\"" << i << "\" addr=\"" << joint_dr << "\" /> \033[0m" << std::endl;
        i++;
    }
